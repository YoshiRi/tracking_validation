...
from evaluation_utils import tracked_obj_to_frame_ground_truth, tracked_obj_to_dynamic_obj, transform_to_matrix
...
class PerceptionEvaluator(Node):
    def __init__(self):
        ...
        result_json_path = "/home/yoshiri/extension_ws/src/tracking_validation/tracking_validation/eval_test/res/result.json"
        t4_dataset_path = ""
        result_archive_path = Path("eval_test")

        self.__timer_group = MutuallyExclusiveCallbackGroup()
        self.__tf_buffer = Buffer(node=self)
        ...
        try:
            ...
            # subscribe two topic simultaneously       
            # qos_profile = QoSProfile(depth=5)
            self.tracks = Subscriber(self, TrackedObjects, "/perception/object_recognition/tracking/objects")
            self.gts = Subscriber(self, TrackedObjects, "/perception/object_recognition/ground_truth/objects")

            self.__sub_perception = ApproximateTimeSynchronizer(
                [self.tracks, self.gts], 10, 0.5)
            self.__sub_perception.registerCallback(self.perception_cb)
    ...
    def perception_cb(self, track_msg: TrackedObjects, gt_msg: TrackedObjects):
        # print("perception_cb")
        self.frame_counter += 1
        from rclpy.time import Duration
    
        try:
            map_to_baselink = self.__tf_buffer.lookup_transform("map", "base_link", gt_msg.header.stamp, timeout=Duration(seconds=1.0))
        except Exception as e:
            print(e)
            # Handle the exception as appropriate, e.g., log a warning, retry, use a default value, etc.
            from builtin_interfaces.msg import Time as TimeMsg
            past_time = Time.from_msg(TimeMsg(sec=gt_msg.header.stamp.sec - 1, nanosec=gt_msg.header.stamp.nanosec))
            map_to_baselink = self.__tf_buffer.lookup_transform("map", "base_link", past_time, timeout=Duration(seconds=1.0))

        ego2map = transform_to_matrix(map_to_baselink)
        # gt and tracks are the same time stamp
        unix_time: int = eval_conversions.unix_time_from_ros_msg(gt_msg.header)
        # 現frameに対応するGround truthを取得
        ground_truth_now_frame = tracked_obj_to_frame_ground_truth(gt_msg, str(self.frame_counter),ego2map=ego2map)
        # ground_truth_now_frame = self.__evaluator.get_ground_truth_now_frame(unix_time)
        if ground_truth_now_frame is None:
            self.__skip_counter += 1
        else:
            print("frame_counter: ", self.frame_counter, unix_time)
            estimated_objects: List[DynamicObject] = tracked_obj_to_dynamic_obj(
                track_msg
            )
...
def main(args=None):
    ...
    try:
        ...
        # executor.spin()
        rclpy.spin(perception_evaluator)
